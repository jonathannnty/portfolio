    ...
    <div className="grid gap-3 sm:grid-cols-3">
      {personas.map(({ key, Icon, label, desc }) => (
        <button
          ...
          className="card group flex flex-col gap-3 p-5 text-left transition-colors hover:border-[color:var(--color-primary-300)] cursor-pointer"
        >
          ...
          <div>
            <p className="font-semibold text-xl text-[color:var(--color-fg)]">{label}</p>
            <p className="mt-1 text-sm text-[color:var(--color-fg-muted)]">{desc}</p>
          </div>
        </button>
      ))}
    </div>
